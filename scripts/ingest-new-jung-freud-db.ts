...
async function main() {
  console.log('====================================================');
  console.log('🚀 INICIANDO INDEXACIÓN EN BD - NUEVOS JUNG & FREUD 🚀');
  console.log('====================================================\n');
  ...
  const jungFreudDocs = index.documentos.filter((d: any) => {
    const normAuthor = (d.autor || '').toLowerCase();
    const normFile = (d.archivo || '').toLowerCase();
    return normAuthor.includes('jung') || normAuthor.includes('freud') || normFile.includes('carl gustav jung') || normFile.includes('sigmund freud');
  });

  const pendingDocs: any[] = [];
  for (const doc of jungFreudDocs) {
  ...
  console.log(`\n📚 Total libros Jung/Freud en índice: ${jungFreudDocs.length}`);
  console.log(`🟡 Nuevos pendientes de indexación profunda en BD: ${pendingDocs.length}\n`);

  if (pendingDocs.length === 0) {
    console.log('✨ Todos los libros de Jung y Freud ya están indexados en la base de datos (READY).');
    await app.close();
    return;
  }
