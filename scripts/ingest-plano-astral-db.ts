...
async function main() {
  console.log('====================================================');
  console.log('🚀 INICIANDO INDEXACIÓN EN BD - PLANO ASTRAL 🚀');
  console.log('====================================================\n');
  ...
  const targetDocs = index.documentos.filter((d: any) => {
    ...
    const categories = (d.categorias || []).map((c: string) => c.toLowerCase());

    const isPlanoAstralFolder = normFile.includes('plano astral/');
    const hasAstralCategory = categories.includes('plano astral') || categories.includes('proyección astral');
    const isKnownAuthor = normAuthor.includes('muldoon') || normAuthor.includes('leadbeater') || normAuthor.includes('monroe') || normAuthor.includes('fox') || normAuthor.includes('ziewe');

    return isPlanoAstralFolder || hasAstralCategory || isKnownAuthor;
  });
  ...
  console.log(`📚 Total libros identificados de Plano Astral: ${targetDocs.length}`);
  console.log(`🟡 Pendientes de indexación en BD: ${pendingDocs.length}\n`);

  if (pendingDocs.length === 0) {
    console.log('✨ Todos los libros de Plano Astral ya están indexados en la base de datos (READY).');
    await app.close();
    return;
  }
