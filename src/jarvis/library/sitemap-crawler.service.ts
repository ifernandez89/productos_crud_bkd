...
export class SitemapCrawlerService {
  ...
  async crawlSource(
    ...
    if (!source.sitemap) {
      this.logger.warn(
        `Source ${source.name} does not have a sitemap configured.`,
      );
      return [];
    }
    ...
    try {
      this.logger.log(
        `Fetching sitemap for ${source.name} at ${source.sitemap}`,
      );
      ...
      this.logger.log(
        `Found ${urls.length} URLs for ${source.name}, prioritized top ${limit}.`,
      );
      return prioritized.slice(0, limit);
    } catch (error: any) {
      this.logger.error(
        `Failed to fetch sitemap for ${source.name}: ${error.message}`,
      );
  ...
  private getScore(url: string, strategy: string): number {
    ...
    switch (strategy) {
      case 'catalog':
        if (
          lowerUrl.includes('producto') ||
          lowerUrl.includes('item') ||
          lowerUrl.includes('p-')
        )
          score += 100;
        if (lowerUrl.includes('oferta') || lowerUrl.includes('promocion'))
          score += 80;
        break;
      case 'healthcare':
        if (
          lowerUrl.includes('especialidad') ||
          lowerUrl.includes('servicio') ||
          lowerUrl.includes('profesional') ||
          lowerUrl.includes('medico')
        )
          score += 100;
        if (
          lowerUrl.includes('guardia') ||
          lowerUrl.includes('emergencia') ||
          lowerUrl.includes('turno')
        )
          score += 120;
        break;
      case 'education':
        if (
          lowerUrl.includes('carrera') ||
          lowerUrl.includes('curso') ||
          lowerUrl.includes('facultad')
        )
          score += 100;
        if (lowerUrl.includes('inscripcion') || lowerUrl.includes('academico'))
          score += 80;
        break;
      case 'corporate':
      default:
        if (
          lowerUrl.includes('nosotros') ||
          lowerUrl.includes('about') ||
          lowerUrl.includes('contacto')
        )
          score += 50;
        if (lowerUrl.includes('servicio') || lowerUrl.includes('solucion'))
          score += 80;
        break;
    }
