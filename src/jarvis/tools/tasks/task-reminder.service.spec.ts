...
describe('TaskReminderService', () => {
  ...
  it('detects a create-task command and persists it', async () => {
    ...
    const reply = await service.handleTaskCommand(
      'crea un pendiente: comprar pan',
      'session-1',
    );
  ...
  it('detects a natural create command that mentions the pending list', async () => {
    taskRepo.createTask.mockResolvedValue({
      id: 2,
      objective: 'Ir al supermercado el sábado',
      status: 'pending',
    });

    const reply = await service.handleTaskCommand(
      'podrías anotar en mi lista de pendientes ir al supermercado el sábado? debo comprar medias y zapatillas',
      'session-1',
    );
    ...
    expect(taskRepo.createTask).toHaveBeenCalledWith({
      sessionId: 'session-1',
      objective:
        'ir al supermercado el sábado? debo comprar medias y zapatillas',
      status: 'pending',
      priority: 'normal',
      category: 'compras',
      project: undefined,
  ...
  it('deletes a specific pending task', async () => {
    ...
    const reply = await service.handleTaskCommand(
      'elimina el pendiente comprar pan',
      'session-1',
    );
  ...
  it('clears the entire pending list', async () => {
    ...
    const reply = await service.handleTaskCommand(
      'limpia toda la lista de pendientes',
      'session-1',
    );
