...
export class GoogleGmailService {
  ...
  async getImportantEmails(maxResults = 10): Promise<string> {
    ...
    if (!gmail)
      return 'No tengo acceso a Gmail. Autenticate primero en /api/jarbees/google/auth.';
    ...
    try {
      ...
      if (messages.length === 0)
        return '📭 No tenés correos importantes no leídos.';
      ...
      const lines = details.map((d, i) => {
        ...
        const from =
          this.getHeader(h, 'From').replace(/<.*>/, '').trim() || 'Desconocido';
  ...
  async summarizeThread(threadId: string): Promise<string> {
    ...
    try {
      ...
      return (
        bodies.slice(0, 3000) || 'No pude extraer el contenido de este hilo.'
      );
    } catch (error) {
  ...
  async searchEmails(query: string, maxResults = 10): Promise<string> {
    ...
    try {
      ...
      const messages = listRes.data.messages ?? [];
      if (messages.length === 0)
        return `📭 No encontré correos para: "${query}"`;
