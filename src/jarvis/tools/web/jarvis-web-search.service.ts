...
export class JarvisWebSearchService {
  ...
  needsWebSearch(message: string): boolean {
    ...
    if (
      ...
      this.logger.log(
        `[needsWebSearch] pedido explícito de búsqueda web detectado.`,
      );
      return true;
    }
    ...
    if (
      /(intendent|gobernador|concejal|concejo|municipalidad|quien gobierna|autoridades|gobierno de parana|gestion municipal)/i.test(
        n,
      )
    ) {
      this.logger.log(
        `[needsWebSearch] gobierno local detectado → buscando en internet`,
      );
      return true;
    }
  ...
  async autoWebSearch(
    ...
    if (category) {
      try {
        const cachePromise = this.contentCache.fetchRelevantContent(
          enrichedQuery,
          category,
          2,
        );
        ...
        this.logger.log(
          `[jarvis:auto_search] caché vacío para "${category}" → WebHelper`,
        );
      } catch (err: any) {
        this.logger.warn(
          `[jarvis:auto_search] error en caché: ${err.message} → fallback`,
        );
      }
    }

    // 2. Fallback: WebHelper con fuentes priorizadas por categoría
    const webHelperTimeout = new Promise<null>((resolve) =>
    ...
    if (result) {
      this.logger.log(
        `[jarvis:auto_search] OK WebHelper (${result.length} chars)`,
      );
      return result;
    }
  ...
  async autoWebSearchWithSources(
    ...
    if (suggestedSources && suggestedSources.length > 0) {
      ...
      if (sourceDefs.length > 0) {
        ...
        if (useful.length > 0) {
          this.logger.log(
            `[domain_search] ${useful.length} resultados de fuentes dirigidas`,
          );
          return useful.join('\n\n---\n\n');
        }
        this.logger.log(
          `[domain_search] fuentes dirigidas vacías → fallback autoWebSearch`,
        );
      }
    }
  ...
  enrichQueryForCategory(query: string, category?: string): string {
    ...
    if (category === 'noticias') {
      const isGeneric =
        /^(noticias?|novedades?|actualidad|que paso|que hay de nuevo|noticias de hoy|ultimas noticias)[\s?¿!¡.]*$/i.test(
          n.trim(),
        );
      if (isGeneric) {
        const localidad = mentionsParana
          ? 'Paraná Entre Ríos'
          : mentionsArgentina
            ? 'Argentina'
            : '';
    ...
    if (category === 'gobierno') {
      const isGeneric =
        /^(gobierno|autoridades|quien gobierna|gestion municipal)[\s?¿!¡.]*$/i.test(
          n.trim(),
        );
      if (isGeneric) {
  ...
  detectCategory(message: string): string | undefined {
    ...
    if (
      /(noticia|novedades|actualidad|resumen|breaking|informa|titulo|tapa|diario|periodico|prensa)/i.test(
        n,
      ) &&
      n.length > 10
    ) {
    ...
    if (
      /(parana\b|ciudad de parana|parque urquiza|costanera|puerto viejo)/i.test(
        n,
      )
    ) {
    ...
    if (
      /(futbol|gol|partido|seleccion|equipo|jugador|campeon|copa|liga|torneo|clasifico|gano|perdio|empato)/i.test(
        n,
      )
    ) {
      return 'deportes';
    ...
    if (
      /(clima|temperatura|lluvia|calor|frio|pronostico|meteorolog|tiempo \(clima\)|despejado|nublado)/i.test(
        n,
      )
    ) {
      return 'clima';
    ...
    if (
      /(\bia\b|inteligencia artificial|machine learning|deep learning|llm\b|openai|chatgpt|gemini\b|claude\b|llama\b|gpt-|gpt4|gpt3|copilot|midjourney|stable diffusion|diffusion model|modelo de lenguaje|red neuronal|transformer\b|hugging face|huggingface)/i.test(
        n,
      )
    ) {
      return 'ia';
    ...
    if (
      /(nestjs|nodejs|node\.js|typescript|javascript|react\b|next\.js|nextjs|vue\b|angular\b|svelte|python\b|rust\b|golang|deno\b|bun\b|npm\b|yarn\b|pnpm|webpack|vite\b|rollup|esbuild|prisma\b|docker\b|kubernetes|k8s|github\b|gitlab|git\b|api rest|graphql|websocket|backend|frontend|framework|libreria|biblioteca|sdk\b|cli\b|dev\.to|medium\.com)/i.test(
        n,
      )
    ) {
      return 'desarrollo';
    ...
    if (
      /(tecnologia|software|hardware|gadget|smartphone|celular|tablet|laptop|procesador|chip\b|apple\b|google\b|microsoft\b|meta\b|amazon\b|app\b|aplicacion)/i.test(
        n,
      )
    ) {
      return 'tecnologia';
    ...
    if (
      /(ciencia|investigacion|estudio|descubr|scientist|paper|journal|nature|conicet)/i.test(
        n,
      )
    ) {
      return 'ciencia';
    ...
    if (
      /(matematica|ecuacion|teorema|demostrac|calculo|algebra|geometria|topologia)/i.test(
        n,
      )
    ) {
      return 'matematicas';
    ...
    if (
      /(fisica|cuantic|particula|cern|relatividad|energia|cosmos|astrofisica)/i.test(
        n,
      )
    ) {
      return 'fisica';
    ...
    if (
      /(musica|cancion|album|artista|concierto|festival|spotify|billboard)/i.test(
        n,
      )
    ) {
      return 'musica';
    ...
    if (
      /(pelicula|film|cine|actor|actriz|director|oscar|estreno|imdb|marvel|mcu|serie)/i.test(
        n,
      )
    ) {
  ...
  async executeSiteSearch(site: string, query: string): Promise<string | null> {
    this.logger.log(
      `[jarvis:site_search] buscando en ${site}: "${query.slice(0, 60)}"`,
    );

    const isHeadlinesQuery =
      /\b(noticias|titulares|novedades|que paso|que hay|actualidad|portada|principales|importantes|recientes|hoy)\b/i.test(
        query,
      );

    if (isHeadlinesQuery) {
      const source = SourceRegistry.getAll().find((s) =>
        s.urlBase.includes(site),
      );
      const targetUrl = source?.urlBase ?? `https://${site}`;
      const limit = this.extractNumberFromQuery(query) ?? 8;

      const headlines = await WebHelper.scrapeHeadlines(
        targetUrl,
        limit,
        source,
      );
      ...
      this.logger.warn(
        `[jarvis:site_search] sin titulares de ${site}, intentando scraping general`,
      );
    }
  ...
  isCurrentEventQuery(message: string): boolean {
    const n = message
      ...
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
    return /(hoy|ayer|esta semana|esta noche|ahora|actualmente|reciente|noticias|novedad|ultimo|ultima|ocurrio|paso hoy|que hay|resultado|partido|gol|marcador|score|precio actual|cotizacion|dolar hoy|quedo|gano|perdio|empato|clasifico|quien gano|como salio|que paso con|revisa|dame las noticias|titulares)/.test(
      n,
    );
  }
  ...
  buildNoEvidenceMessage(query: string, site?: string): string {
    ...
    return [
      ...
      site
        ? `- Consultar directamente: https://${site}`
        : `- Reformular la pregunta o intentar en un momento`,
