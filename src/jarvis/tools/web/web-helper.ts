...
export class WebHelper {
  ...
  static async search(
    ...
    if (!searchResults.length) {
      WebHelper.logger.warn(
        `[WebHelper] sin resultados DuckDuckGo para: "${query.slice(0, 60)}"`,
      );
      return null;
    }
    ...
    WebHelper.logger.log(
      `[WebHelper] scrapeando ${urls.length} URLs de DuckDuckGo`,
    );
    ...
    const elapsed = Date.now() - startTime;
    WebHelper.logger.log(
      `[WebHelper] resultado final en ${elapsed}ms (${usefulScraped.length} artículos)`,
    );
  ...
  static async scrapeHeadlines(
    ...
    try {
      ...
      WebHelper.logger.log(
        `[WebHelper:headlines] ${top.length} titulares extraídos de ${hostname}`,
      );
      return `**Titulares verificados de ${hostname}:**\n${formatted}\n\n_(Fuente: https://${hostname} — consultado en tiempo real)_`;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      WebHelper.logger.warn(
        `[WebHelper:headlines] error scraping ${url}: ${msg}`,
      );
      return null;
    }
  }
  ...
  static async scrapeUrlWithSelectors(
    ...
    try {
      ...
      WebHelper.logger.log(
        `[WebHelper] scraped ${truncated.length} chars de ${hostname}`,
      );
      return `${truncated}\n\n_(Fuente: ${hostname})_`;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
    ...
    try {
      ...
      const source = SourceRegistry.getAll().find((s) => {
        try {
          ...
          return (
            hostname === sHost ||
            hostname.endsWith('.' + sHost) ||
            sHost.endsWith('.' + hostname)
          );
        } catch {
          return false;
        }
      ...
      if (source) {
        WebHelper.logger.log(
          `[WebHelper] URL de fuente conocida detectada: ${source.name}. Usando selectores optimizados.`,
        );
        const content = await WebHelper.scrapeUrlWithSelectors(
          url,
          contextQuery ?? '',
          source,
        );
  ...
  private static async ddgSearch(
    ...
    try {
      ...
      WebHelper.logger.log(
        `[WebHelper] DuckDuckGo: ${results.length} resultados`,
      );
      return results;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
