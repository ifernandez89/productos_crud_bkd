...
- **🌐 Regla Estricta de Idioma y Traducción** (`src/jarvis/prompt/jarvis-prompt-builder.service.ts` y `src/jarvis/jarvis.service.ts`): Se reforzó la Regla General 1 del System Prompt y el encabezado del contexto RAG (`### DOCUMENTOS DE LA BIBLIOTECA PERSONAL (RAG)`) exigiendo explícitamente responder 100% en español (es-AR) y traducir cualquier fragmento recuperado que esté en inglés u otros idiomas (ej. obras de Monroe, Fox, Muldoon o Ziewe).
- **Impacto**: Todas las consultas sobre libros de la biblioteca personal reciben `intent: 'RAG', confidence: 'high'` instantáneamente, sin pasar por el LLM clasificador, y el modelo responde limpiamente utilizando los fragmentos del contexto RAG traducidos siempre al español. Tiempo de routing: ~0ms vs ~3-8s anterior.
